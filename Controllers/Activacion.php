...
require_once('Libraries/Core/ActivacionValidators.php');
require_once('Models/LogJsonActivacionModel.php');
...
class Activacion extends Controllers {
    ...
    public function __construct() {
        ...
        try {
            ...
            $this->processInvoiceBatch($this->activaciones);
    ...
    public function activacion() {
        ...
        if (!empty($this->logId)) {
            ...
            $logJsonModel = new LogJsonActivacionModel();
    ...
    private function initializeRequest() {
		...
		$this->activaciones = (isset($data->data) && is_array($data->data)) ? $data->data : NULL;
		
        if (empty($this->activaciones)) {
           $this->sendError(400, "El JSON debe contener un array de 'data'.");
        }

        $logJsonModel = new LogJsonActivacionModel();
        $this->logId = $logJsonModel->insertLogRequest($postdata, 'ACTIVACIÓN', $this->empresa);
    }

    // El corazón de la nueva arquitectura.
    private function processInvoiceBatch(array $activaciones) {
        
        $this->total = count($activaciones);
        ...
        try {
            ...
            // =============================================================
             foreach ($activaciones as $activacion) {
                ...
                ActivacionValidators::validateActivacionCompleta($activacion);
            }
            ...
            foreach ($activaciones as $activacion) {
                try {
                    ...
                    $this->model->procesarActivacionIndividual($activacion);

                    // Si no hay excepción, la venta fue exitosa.
                    $this->procesadas++;
                    $this->resultados[] = [
                        "status" => true,
                        "message" => "Activación procesada correctamente."
                    ];
                } catch (Exception $e) {
